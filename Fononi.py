...
# Set how many iterationf of refinement around the cylinder should be done (here 2)
[(mesh, bndry)] = Mymesh_fononi.generate(2) 
...
#bcm_walls = DirichletBC(W.sub(0).sub(1), 0, bndry, 3) # Dirichlet BC on walls. Full slip. Not used.
...
#bce_right = DirichletBC(W.sub(1), cold, bndry, 2) # BC for energy density e; not used
...
beta = 10000 # Parameter beta from stabilisation term in Nitsche method. The higher, the larger the effect of the stabilisation.
...
edgelen = mesh.hmin()                      # Lenght of the shortest edge in the triangulation
I = Identity(mesh.geometry().dim())        # Identity tensor of given dimension, here matrix.
...
def f(v,u, e) :
    ...
    #return 0 # for Guyer-Krumhansl model

# Expressions for enforcing Nitsche BC. Integration over ds(5)works on cylinder; ds(3) on the walls:    
def N0(m,m_) :
    if Theta == 0.0:
        return 0
    else:
        return (Theta/(gamma*(1.0-Theta))*inner(vt(m,n),vt(m_,n)))*ds(5) #+ (Theta/(gamma*(1.0-Theta))*inner(vt(m,n),vt(m_,n)))*ds(3) 
    
def N1(e,m,m_):
    w_ = TestFunction(W)
    return (-inner(dot(StressT(e,m,m,e),n),n)*inner(m_,n))*ds(5) #+ (-inner(dot(StressT(e,m,m,e),n),n)*inner(m_,n))*ds(3) 

def N2(e_,m_,m,mk, ek):
    return (inner(dot(StressT(e_,m_,mk, ek),n),n)*inner(m,n))*ds(5) #+ (inner(dot(StressT(e_,m_,mk, ek),n),n)*inner(m,n))*ds(3) 

def Nstab(m,m_):
    w_ = TestFunction(W)
    return beta*viscosity/edgelen * (inner(vn(m,n),vn(m_,n))*ds(5)) #+ beta*viscosity/edgelen * (inner(vn(m,n),vn(m_,n))*ds(3))
...
# Multiply the above equation by test functions and integrate over space to get the full space-time formulation. 
mdot=Constant(1.0/dt)*inner(m-m0,m_)*dx
edot=Constant(1.0/dt)*inner(e-e0,e_)*dx
F = mdot + edot + theta*F1 + (1.0-theta)*F0 

J = derivative(F, w)    # Specifying this derivative accelerates computation
...
name="phonons_10000_2" # Named as follows: name_beta_refinement level
out_file=dict()
for i in ['m', 'e'] :
...
while t < t_end:
    ...
    wallcond.append((t,math.sqrt(assemble(through(w.sub(0),n)*ds(5)))))
    incond.append((t,math.sqrt(assemble(through(w.sub(0),n)*ds(1)))))
...
np.savetxt('wallcond_10000_2.txt',wallcond)
np.savetxt('incond_10000_2.txt',incond)
